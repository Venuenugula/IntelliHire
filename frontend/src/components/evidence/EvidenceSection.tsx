...
function EvidenceCard({ ev }: { ev: EvidenceObject }) {
  ...
  return (
    <div className="card card-hover relative overflow-hidden p-5">
      ...
      <div className="relative flex items-start gap-4">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            ...
            <h3 className="font-semibold text-gray-900">{sourceLabel(ev.source)}</h3>
            <span
              className="chip ml-1 px-2 py-0.5 text-[10px] uppercase tracking-wide text-gray-500"
              title="How much this source is weighted for reliability"
            >
              {Math.round(ev.reliability * 100)}% reliable
            </span>
          </div>
          {ev.summary && <p className="text-sm text-gray-600">{ev.summary}</p>}
        </div>
      ...
      {ev.error ? (
        <p className="relative mt-3 text-xs text-amber-600">Source could not be fully analyzed: {ev.error}</p>
      ) : (
        <>
          {ev.skills.length > 0 && (
          ...
          {ev.signals.length > 0 && (
            <dl className="relative mt-4 space-y-1.5">
              {ev.signals.map((sig) => (
                <div key={sig.label} className="flex items-start justify-between gap-3 border-b border-gray-100 pb-1.5 last:border-0">
                  <dt className="text-xs uppercase tracking-wide text-gray-400">{sig.label.replace(/_/g, " ")}</dt>
                  <dd className="text-right text-xs text-gray-700">{sig.detail}</dd>
                </div>
              ))}
            </dl>
          )}
          ...
          {ev.highlights.length > 0 && (
            <div className="relative mt-4">
              <p className="mb-1 text-[11px] font-medium uppercase tracking-wide text-emerald-600">Highlights</p>
  ...
  return (
    <section id="evidence" className="mt-7 scroll-mt-24">
      <div className="mb-3 flex items-baseline gap-3">
        ...
        <span className="text-xs text-gray-400">{evidence.length} source{evidence.length === 1 ? "" : "s"} analyzed</span>
      </div>
