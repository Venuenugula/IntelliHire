...
POST /v2/graph/build wires the real ``GraphBuilder`` (Evidence -> ledger -> entity
resolution -> construction -> dedup -> inference -> fusion -> CandidateGraph) and
caches the result in the ``graph_registry``. The GET routes answer the layer's
target questions: what skills does the candidate actually have, what evidence
proves each, how strong is the confidence, what projects/technologies connect, and
what inferred capabilities exist beyond explicit claims.
"""
...
@router.post(
    ...
    description=(
        ...
        "relationship inference, confidence fusion). The server assigns the "
        "graph_id and caches the result for the query endpoints."
    ),
)
